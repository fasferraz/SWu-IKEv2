...
class swu():
    ...
    def __init__(self, source_address,epdg_address,apn,modem,default_gateway):
    ...
    def set_variables(self):
        ...
        self.set_identification(IDI,ID_RFC822_ADDR,'0' + DEFAULT_IMSI + '@nai.epc.mnc' + DEFAULT_MNC + '.mcc' + DEFAULT_MCC + '.3gppnetwork.org')
        self.set_identification(IDR,ID_FQDN, self.apn + '.apn.epc.mnc' + DEFAULT_MNC + '.mcc' + DEFAULT_MCC + '.3gppnetwork.org')
    ...
    def get_identity(self):
            imsi = get_imsi(self.com_port)
            self.set_identification(IDI,ID_RFC822_ADDR,'0' + imsi + '@nai.epc.mnc' + DEFAULT_MNC + '.mcc' + DEFAULT_MCC + '.3gppnetwork.org')
                        ...
                        if i[1][3] in (AKA_Challenge, AKA_Reauthentication):
                            ...
                            if (RAND is not None and AUTN is not None) or (VECTOR is not None and ENCR_DATA is not None):
                                if RAND is not None and AUTN is not None:
                                    ...
                                    res,ck,ik = get_res_ck_ik(self.com_port,toHex(RAND),toHex(AUTN))
                                    print('RES',res)
                                    print('CK',ck)
                                    print('IK',ik)
...
def main():
    ...
    parser = OptionParser()    
    parser.add_option("-m", "--modem", dest="modem", default=DEFAULT_COM, help="modem port (i.e. COMX, or /dev/ttyUSBX)")
    ...
    a = swu(options.source_addr,options.destination_addr,options.apn,options.modem,options.gateway_ip_address)
    ...
    #a.userplane_mode = NAT_TRAVERSAL
